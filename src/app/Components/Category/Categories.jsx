...
import React, { useMemo, useState, useEffect } from 'react';
import { useSearchParams, useRouter } from 'next/navigation';
import productsData from '@/Data/ProductData.json'; // Assuming products are imported like this
import Link from 'next/link';
...
const Categories = () => {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('favoritePlugUser');
    console.log('Categories: favoritePlugUser from localStorage:', stored); // Debug
    if (stored) {
      try {
        const parsed = JSON.parse(stored);
        const foundUserId = parsed?.id || parsed?.email;
        console.log('Categories: Parsed userId:', foundUserId); // Debug
        setUserId(foundUserId);
      } catch (e) {
        console.error('Categories: Error parsing stored user:', e);
        setUserId(null); // Ensure userId is null if parsing fails
      }
    } else {
      console.log('Categories: No favoritePlugUser found in localStorage.');
    }
  }, []);

  // console.log('Categories: Rendering with current userId state:', userId); // Debug before hook call
  const { addToWishlist, removeFromWishlist, isWishlisted, isMounted } = useWishlist(userId);
  ...
  useEffect(() => {
    setLoading(true); // Set loading to true when category or page changes
    const timeout = setTimeout(() => setLoading(false), 1000); // Shorter timeout for testing
    return () => clearTimeout(timeout);
  }, [category, page]);
        ...
        <main className="grid max-sm:grid-cols-3 max-sm:gap-1 max-md:grid-cols-3 grid-cols-4 lg:grid-cols-4 overflow-x-auto gap-4 p-3 w-full scrollbar-hide">
          {currentPageProducts.map((product) => {
            if (!product || typeof product.id === 'undefined') {
              console.warn('Categories: Rendering a product without id or product itself is undefined', product);
              return null; // Skip rendering this product
            }
            const isInWishlist = isWishlisted(product.id);
            // console.log(`Categories [${product.name}]: isInWishlist: ${isInWishlist}, isMounted (from hook): ${isMounted}`); // Debug
            ...
            return (
              <div key={product.id} className="relative">
                {isMounted && ( // Only show button if hook is ready
                  <button
                    onClick={() => {
                      console.log(`Categories [${product.name}]: Heart button clicked. isInWishlist: ${isInWishlist}`); // Debug
                      if (isInWishlist) {
                  ...
                  <div className="px-4 py-5 flex flex-col rounded-lg shadow-md bg-white hover:shadow-lg transition-shadow duration-300">
                    <div className="flex justify-center items-center mb-1">
                      <img
                        src={product.image}
                        alt={product.name}
                        className="max-sm:h-[40px] h-[100px] object-fit rounded-lg" // Image size was different here
                      />
                    </div>
                    <div className="w-[100%] max-sm:w-[100%] flex flex-col justify-center items-start"> {/* was 50% width */}
                      <h1 className="text-sm max-sm:text-[0.7rem] font-semibol cursor-pointer">
                        {product.name.slice(0, 11)}...
                      </h1>
                      <div className="flex max-sm:items-center gap-2 max-sm:hidden">
                       ...
                       {/* For smaller screens, maybe show price too if it was hidden */}
