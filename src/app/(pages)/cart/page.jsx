...
//                 <Image src={displayImage} alt={item.product.name} width={96} height={96} className="rounded-md object-cover h-24 w-24 max-sm:h-20 max-sm:w-20" />
...
const CartPage = () => {
  ...
  // We removed the 'displayImage' logic from here
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="w-full lg:w-2/3 space-y-4">
          {cart.items.map(item => {
            ...
            // --- THIS IS THE FIX ---
            // We moved the logic INSIDE the loop and used 'item.product'
            const displayImage = item.product.images && item.product.images.length > 0
              ? item.product.images[1]
              : '/Images/placeholder.png'; // Make sure this placeholder exists!
              
              console.log("FINAL IMAGE URL BEING USED:", displayImage); // Safety check
            const handleQuantityChange = (newQuantity) => {
            ...
            return (
              <div key={item.id} className="flex items-center bg-white p-4 rounded-lg shadow-sm border">
                ...
                {/* Now the <Image> component gets the correct image source for each item */}
                <Image 
                  src={displayImage} 
